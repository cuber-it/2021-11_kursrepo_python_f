...
class Messgeraet_Basis:
    ...
    def set_values(self, values=[]):
        if not isinstance(values,list):
            raise TypeError("muss eine Liste sein")        
        if values == []:
            self.status = STATUS_INIT
        else: 
...
if __name__ == "__main__":
    ...
    class MG_Basis_Tests(unittest.TestCase):
        ...
        def test_init(self):            # methoden für die Tests müssen mit test anfangen
        ...
        #        self.b.set_values("1, 2, 3")
            self.assertRaises(TypeError, self.b.set_values, "1, 2, 3")
